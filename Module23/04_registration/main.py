...
for i_line in registration_file:
    ...
    # TODO, функция chek должна запускаться внутри блоков try и except.
    #  В функции chek блоков try и except быть не должно.
    #  Функция должна проверять строку на наличие ошибок и вызывать ошибку (raise).
    chek(i_line)
