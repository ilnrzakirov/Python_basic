...
class QHof:
    ...
    def __next__(self):
        # TODO, введу постоянного роста нашей последовательности,
        ...
        if len(self.Q) <= self.count + 1:
            ...
            self.Q.append(q)
            self.count += 1
            # TODO, можно перенести эту строку в самое начало текущего метода
            #  В таком случае, в остальных местах кода +1 можно убрать
        # , стоит увеличивать count на 1.
        return q  # TODO , возвращать предлагаю элемент который добавили в список. q добавляем не всегда =)
