...
# , для вывода как в примере, стоит попробовать передавать рекурсивно число,
...
def print_struct(struct, n):
    ...
    for key, value in struct.items():  # Где то здесь ошибка
        # , проверять является ли словарём или нет необходимо значение, а не ключ =)
...
for _ in range(n):
    ...
    print_struct(copy_site, 0)  # , возможно, стоит начинать с "1"
